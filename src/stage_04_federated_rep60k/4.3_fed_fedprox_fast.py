...
def train_client_fedprox(client_id, global_state, train_loader, val_loader, model_type, device, epochs=2, lr=3e-4, mu=MU_PROXIMAL):
...
def run_fedprox():
    logger.info(f"=== STARTING STAGE 4.3: FEDPROX [FAST TRACK, mu={MU_PROXIMAL}] ===")
    ...
    for r in range(1, rounds + 1):
        ...
        logger.info(f"\n--- [FedProx Fast] COMMUNICATION ROUND {r:02d}/{rounds:02d} ---")
        ...
        logger.info(f"[FedProx Fast] Round {r:02d} Complete ({round_time:.1f}s) -> Global Test B Macro F1: {test_b_f1:.4f}")
    ...
    logger.info(f"\n[FedProx Fast] All {rounds} Rounds Completed in {total_time:.2f}s.")
    ...
    logger.info(f"Saved FedProx Checkpoint to: {OUT_CKPT}")
