...
def train_centralized():
    logger.info("=== STARTING STAGE 4.1: CENTRALIZED ORACLE [FAST TRACK - 60k SAMPLES] ===")
    ...
    train_loaders, val_loaders, _ = load_client_loaders(batch_size=256)
    ...
    logger.info(f"Pooled Training Samples:   {len(pooled_train_ds):,}")
    logger.info(f"Pooled Validation Samples: {len(pooled_val_ds):,}")
    
    central_train_loader = DataLoader(pooled_train_ds, batch_size=256, shuffle=True, pin_memory=torch.cuda.is_available())
    central_val_loader = DataLoader(pooled_val_ds, batch_size=256, shuffle=False, pin_memory=torch.cuda.is_available())
    ...
    for epoch in range(1, epochs + 1):
        ...
        logger.info(f"[Centralized Fast] Epoch {epoch:02d}/{epochs:02d} - Loss: {avg_loss:.4f} | Val Acc: {val_acc:.4f} | Val Macro F1: {val_f1:.4f}")
    ...
    logger.info(f"Saved Centralized Checkpoint to: {OUT_CKPT}")
