...
def train_client_dafl(client_id, global_state, base_state_tensors, train_loader, val_loader, model_type, device, epochs=2, lr=3e-4, lambda_reg=LAMBDA_ANCHOR):
...
def run_dafl():
    logger.info(f"=== STARTING STAGE 4.5: DUAL-ANCHOR FL (DAFL - OURS) [FAST TRACK, lambda={LAMBDA_ANCHOR}] ===")
    ...
    for r in range(1, rounds + 1):
        ...
        logger.info(f"\n--- [DAFL Fast] COMMUNICATION ROUND {r:02d}/{rounds:02d} ---")
        ...
        logger.info(f"[DAFL Fast] Round {r:02d} Complete ({round_time:.1f}s) -> Global Test B Macro F1: {test_b_f1:.4f}")
    ...
    logger.info(f"\n[DAFL Fast] All {rounds} Rounds Completed in {total_time:.2f}s.")
    ...
    logger.info(f"Saved DAFL Checkpoint to: {OUT_CKPT}")
