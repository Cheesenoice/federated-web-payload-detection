...
class HybridEnsembleModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.alpha * prob_base + (1.0 - self.alpha) * prob_fed
...
def run_hybrid_ensemble():
    logger.info("=== STARTING STAGE 4.6: HYBRID ENSEMBLE INTELLIGENCE [FAST TRACK] ===")
    ...
    acc_b, f1_b, _, _, _ = evaluate_ensemble_loader(ensemble, global_b_loader, device)
    acc_ood, f1_ood, _, _, _ = evaluate_ensemble_loader(ensemble, ood_loader, device)
    
    logger.info(f"[Hybrid Ensemble Fast] Global Network Test B -> Macro F1: {f1_b:.4f}")
    logger.info(f"[Hybrid Ensemble Fast] External OOD CSIC     -> Macro F1: {f1_ood:.4f}")
