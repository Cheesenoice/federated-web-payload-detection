...
def run_fedavgm():
    logger.info(f"=== STARTING STAGE 4.4: FEDAVGM SERVER MOMENTUM [FAST TRACK, beta={BETA_MOMENTUM}] ===")
    ...
    for r in range(1, rounds + 1):
        ...
        logger.info(f"\n--- [FedAvgM Fast] COMMUNICATION ROUND {r:02d}/{rounds:02d} ---")
        ...
        logger.info(f"[FedAvgM Fast] Round {r:02d} Complete ({round_time:.1f}s) -> Global Test B Macro F1: {test_b_f1:.4f}")
    ...
    logger.info(f"\n[FedAvgM Fast] All {rounds} Rounds Completed in {total_time:.2f}s.")
    ...
    logger.info(f"Saved FedAvgM Checkpoint to: {OUT_CKPT}")
