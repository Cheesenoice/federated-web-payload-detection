...
OUT_CKPT = os.path.join(MODELS_DIR, "W_dafl.pt")
OUT_HISTORY_CSV = os.path.join(REPORTS_DIR, "history_dafl.csv")

LAMBDA_ANCHOR = 0.02

def train_client_dafl(client_id, global_state, base_state_tensors, train_loader, val_loader, model_type, device, epochs=3, lr=3e-4, lambda_reg=LAMBDA_ANCHOR):
    """Executes local client training with Dual-Anchor Regularization: L = L_ce + (lambda/2) * ||w - W_base||^2."""
    ...
    for epoch in range(epochs):
        for x, y in train_loader:
            ...
            with torch.amp.autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", enabled=torch.cuda.is_available()):
                ...
                # Compute Dual-Anchor Penalty to Pre-trained W_base Anchor
                anchor_loss = 0.0
                for name, param in local_model.named_parameters():
                    if name in base_state_tensors:
                        anchor_loss += torch.sum((param - base_state_tensors[name]) ** 2)
                        
                total_loss = ce_loss + (lambda_reg / 2.0) * anchor_loss
...
def run_dafl():
    logger.info(f"=== STARTING STAGE 4.5: DUAL-ANCHOR FEDERATED LEARNING (DAFL - OURS) [lambda={LAMBDA_ANCHOR}] [TURBOCHARGED] ===")
    if os.path.exists(OUT_CKPT) and os.path.exists(OUT_HISTORY_CSV):
        logger.info(f"--> [SKIP] DAFL checkpoint already exists at: {OUT_CKPT}. Skipping training.")
        return
    ...
    logger.info(f"Initialized DAFL Global Model from W_base ({model_type.upper()})")
    
    # Freeze W_base anchor tensors in memory
    base_state_tensors = {k: v.clone().detach().to(device) for k, v in global_state.items()}
    ...
    for r in range(1, rounds + 1):
        ...
        logger.info(f"\n--- [DAFL lambda={LAMBDA_ANCHOR}] COMMUNICATION ROUND {r:02d}/{rounds:02d} ---")
        
        client_states = {}
        client_val_f1s = {}
        
        # Local DAFL training across all 6 clients
        for i in range(1, 7):
            cid = f"client_{i}"
            c_state, c_val_f1 = train_client_dafl(
                cid, global_state, base_state_tensors, train_loaders[cid], val_loaders[cid],
                model_type, device, epochs=local_epochs, lr=3e-4, lambda_reg=LAMBDA_ANCHOR
            )
            client_states[cid] = c_state
            client_val_f1s[cid] = c_val_f1
            logger.info(f"[{cid}] Local DAFL {local_epochs} Epochs Done -> Local Val F1: {c_val_f1:.4f}")
        ...
        logger.info(f"[DAFL] Round {r:02d} Complete ({round_time:.1f}s) -> Global Test B Acc: {test_b_acc*100:.2f}% | Global Test B Macro F1: {test_b_f1:.4f}")
        ...
        save_round_checkpoint("DAFL", r, global_state, model_type, log_entry)
        
    total_time = time.time() - start_total_time
    logger.info(f"\n[DAFL] All {rounds} Rounds Completed in {total_time:.2f}s.")
    
    # Save History CSV
    df_history = pd.DataFrame(round_history)
    df_history.to_csv(OUT_HISTORY_CSV, index=False)
    logger.info(f"Saved DAFL Round History to: {OUT_HISTORY_CSV}")
    ...
    logger.info(f"Saved DAFL Global Checkpoint to: {OUT_CKPT}")
