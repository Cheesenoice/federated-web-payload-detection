...
OUT_CKPT = os.path.join(MODELS_DIR, "W_fedavgm.pt")
OUT_HISTORY_CSV = os.path.join(REPORTS_DIR, "history_fedavgm.csv")
...
def run_fedavgm():
    logger.info(f"=== STARTING STAGE 4.4: FEDAVGM SERVER MOMENTUM (Hsu et al., 2019) [beta={BETA_MOMENTUM}] [TURBOCHARGED] ===")
    if os.path.exists(OUT_CKPT) and os.path.exists(OUT_HISTORY_CSV):
        logger.info(f"--> [SKIP] FedAvgM checkpoint already exists at: {OUT_CKPT}. Skipping training.")
        return
    ...
    logger.info(f"Initialized FedAvgM Global Model from W_base ({model_type.upper()})")
    
    # Initialize Server-side Velocity Buffer
    ...
    for r in range(1, rounds + 1):
        ...
        logger.info(f"\n--- [FedAvgM beta={BETA_MOMENTUM}] COMMUNICATION ROUND {r:02d}/{rounds:02d} ---")
        ...
        w_avg = aggregate_weighted_parameters(client_states, client_sample_counts)
        
        # Apply Server-Side Momentum
        new_global_state = {}
        for key in global_state.keys():
            delta_w = global_state[key].to(torch.float32) - w_avg[key].to(torch.float32)
            velocity_buffer[key] = BETA_MOMENTUM * velocity_buffer[key] + delta_w
            new_tensor = global_state[key].to(torch.float32) - (SERVER_LR * velocity_buffer[key])
            new_global_state[key] = new_tensor.to(global_state[key].dtype)
        ...
        logger.info(f"[FedAvgM] Round {r:02d} Complete ({round_time:.1f}s) -> Global Test B Acc: {test_b_acc*100:.2f}% | Global Test B Macro F1: {test_b_f1:.4f}")
        ...
        save_round_checkpoint("FedAvgM", r, global_state, model_type, log_entry)
        
    total_time = time.time() - start_total_time
    logger.info(f"\n[FedAvgM] All {rounds} Rounds Completed in {total_time:.2f}s.")
    
    # Save History CSV
    df_history = pd.DataFrame(round_history)
    df_history.to_csv(OUT_HISTORY_CSV, index=False)
    logger.info(f"Saved FedAvgM Round History to: {OUT_HISTORY_CSV}")
    ...
    logger.info(f"Saved FedAvgM Global Checkpoint to: {OUT_CKPT}")
