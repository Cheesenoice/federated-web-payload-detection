...
def plot_convergence_curves():
    ...
    plt.title("Federated Convergence on Global Test B across 10 Communication Rounds", fontsize=13, fontweight="bold")
...
def plot_ood_bar_chart(summary_df):
    ...
    bars = plt.bar(algos, ood_f1s, color=["gray", "silver", "steelblue", "skyblue", "lightcoral", "darkseagreen", "mediumorchid", "gold"])
    plt.title("Zero-Shot OOD Robustness Comparison (External CSIC 2010 Benchmark)", fontsize=13, fontweight="bold")
...
def run_master_benchmark():
    logger.info("=== STARTING STAGE 4.7: MASTER FEDERATED BENCHMARK & COMPARISON ===")
    device = get_device()
    logger.info(f"Computing Device: [{device}]")
    
    # 1. Load Test Holdouts (Batch size 512)
    client_test_loaders, global_b_loader, ood_loader = load_test_holdouts(batch_size=512)
    
    # 2. Candidate Models Suite
    models_to_eval = [
    ...
    for name, ckpt_path, eval_mode in models_to_eval:
        ...
        # A. Evaluate across all 6 Local Client Test Sets
        client_f1s = []
        for i in range(1, 7):
            cid = f"client_{i}"
        ...
        # B. Evaluate on Global Test B (354,808 samples)
        acc_b, f1_b, rep_b = evaluate_model_on_loader(m, global_b_loader, device, is_ensemble=is_ens)
        
        # C. Evaluate on External OOD CSIC 2010 (122,130 samples)
