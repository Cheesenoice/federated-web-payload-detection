...
OUT_CKPT = os.path.join(MODELS_DIR, "model_transformer.pt")
...
def train():
    logger.info("=== STARTING STAGE 3.3.3: TRAIN TRANSFORMER ENCODER (SecBERT/MiniLM Architecture) ===")
    ...
    logger.info(f"Transformer Encoder Trainable parameters: {total_params:,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        logger.info(f"[Transformer] Epoch {epoch:02d}/{epochs:02d} - Loss: {avg_loss:.4f} | Val Acc: {val_acc:.4f} | Val Macro F1: {val_f1:.4f}")
    ...
    logger.info(f"Transformer Training finished in {elapsed:.2f}s. Saved checkpoint to {OUT_CKPT}")
