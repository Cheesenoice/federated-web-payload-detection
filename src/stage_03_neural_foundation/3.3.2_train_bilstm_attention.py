...
OUT_CKPT = os.path.join(MODELS_DIR, "model_bilstm.pt")
...
def train():
    logger.info("=== STARTING STAGE 3.3.2: TRAIN Bi-LSTM WITH SELF-ATTENTION ===")
    ...
    model = get_neural_model("bilstm", num_classes=4).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f"Bi-LSTM Trainable parameters: {total_params:,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        logger.info(f"[Bi-LSTM] Epoch {epoch:02d}/{epochs:02d} - Loss: {avg_loss:.4f} | Val Acc: {val_acc:.4f} | Val Macro F1: {val_f1:.4f}")
    ...
    logger.info(f"Bi-LSTM Training finished in {elapsed:.2f}s. Saved checkpoint to {OUT_CKPT}")
